...
* We only use the least significant 32 bits of the 64 bits for tags
  such that we maintain the description in the book.
...
A header has the form 
00000000000000000000000000000000ttttttttnnnnnnnnnnnnnnnnnnnnnngg
where tttttttt is the block tag, all 0 for cons cells
nn....nn is the block length (excluding header)
gg       is the block's color

The first 32 0's represent the extension into 64 bit which is not
used.
...
#elif defined(ENV64)
  #define IsInt(v) (((v)&1)==1)
  #define Tag(v) ((((v)<<1)|1)&0xFFFFFFFF)  /* Truncate to 32 bit */
  #define Untag(v) (((v)>>1)&0xFFFFFFFF)    /* Truncate to 32 bit */
#endif
...
#elif defined(ENV64)
  #define BlockTag(hdr) (((hdr)>>24)&0xFFFFFFFF)
  #define Length(hdr)   (((hdr)>>2)&0x003FFFFF)
  #define Color(hdr)    ((hdr)&3)
  #define Paint(hdr, color)  (((hdr)&(0xFFFFFFFC))|(color))
#endif
...
word* allocate(unsigned int tag, unsigned int length, word s[], word sp);

// The machine: execute the code starting at p[pc] 

int execcode(word p[], word s[], int iargs[], int iargc, int /* boolean */ trace) {
...
void heapStatistics() {
	...
	while (heapPtr < afterHeap) {
		if (Length(heapPtr[0]) > 0) {
			...
			orphans++;
		//printf("heapPtr.l = %lld \n", Length(*heapPtr));
		word* nextBlock = heapPtr + Length(heapPtr[0]) + 1;
		//printf("nextBlock.l = %lld \n", Length(*nextBlock));
	...
	while (freePtr != 0) {
		...
		if (freePtr < heap || afterHeap < freePtr + length + 1) {
			printf("HEAP ERROR: freelist item %d (at heap[" WORD_FMT "], length %d) is outside heap\n",
...
void mark(word* block) {
	if (Color(block[0]) == 2) return;
	block[0] = Paint(*block, 0x2);
	if (!IsInt(block[1]) && (block[1] != 0)) { mark((word*)block[1]); }
	if (!IsInt(block[2]) && (block[2] != 0)) { mark((word*)block[2]); }
}

void markPhase(word s[], word sp) {
  for (int i = 0; i <= sp; i++) {
	  if (IsInt(s[i]) || (s[i]==0)) continue;
	  mark((word*)s[i]);
  }
}
void sweepPhase() {
	...
	while (heapPtr < afterHeap) {
		word* c = (word*)heapPtr;
		word* heapPtrOld = heapPtr;
		heapPtr += Length(heapPtr[0]) + 1;

		if (Length(c[0]) <= 1) {
			if (lastWasFree) {
				freelist[0] = mkheader(Tag(freelist[0]), Length(c[0]) + Length(freelist[0]) + 1, Blue);
			}
			else { lastWasFree = 0; }
			continue;
		}
		...
		if (firstTime) {
			freelist = c;
			c[0] = mkheader(Tag(c[0]), Length(c[0]), Blue);
			c[1] = 0;
			firstTime = 0;
		}
		else {
...
void collect(word s[], word sp) {
  //printf("\nStart mark \n");
  markPhase(s, sp);
  //printf("Done with mark \n");
  heapStatistics();
  sweepPhase();
  //printf("Done with sweep \n");
  heapStatistics();
  //printf("Done with collect \n");
}
word* allocate(unsigned int tag, unsigned int length, word s[], word sp) {
	int attempt = 1;
	do {
