...
type expr<'a> = 
  ...
  | Fst of expr<'a> * 'a option
  | Snd of expr<'a> * 'a option
and valdec<'a> =
...
let ppProg fPP p : string =
  ...
  let rec ppExpr' i e =
    ...
    | Fst(Pair(e1, e2, aOpt), aOpt2) -> (ppExpr' i e1) + (fPP aOpt) + (fPP aOpt2)
    | Snd(Pair(e1, e2, aOpt), aOpt2) -> (ppExpr' i e2) + (fPP aOpt) + (fPP aOpt2)
    | Fst(_, _) -> failwith "wrong fst argument"
    | Snd(_, _) -> failwith "wrong snd argument"
...
let tailcalls p : program<'a> =
  let rec tc' tPos e =
    ...
    | Fst(t,aOpt2) -> Fst((tc' false t) ,aOpt2)
    | Snd(t,aOpt2) -> Snd((tc' false t) ,aOpt2)
...
let rec freevars e : string Set =
  ...
  | Fst(Pair(e1, e2, _), _) -> (freevars e1)
  | Snd(Pair(e1, e2, _), _) -> (freevars e2)
  | Fst(_, _) -> failwith ("error on fst")
  | Snd(_, _) -> failwith ("error on snd")
