...
let rec evalExpr (env : value env) (e : expr<typ>)
  ...
  | Pair(e1, e2, aOpt) ->
    evalExpr env e1 (fun v1 ->
        evalExpr env e2
            (fun v2 -> PairV(v1, v2) |> cont)
            econt)
        econt
    | Fst(Pair(e1, e2, _), _) -> evalExpr env e1 (fun v -> cont v) econt
    | Snd(Pair(e1, e2, _), _) -> evalExpr env e2 (fun v -> cont v) econt
    | Fst(_, _) -> Abort "evalExpr fst: not a pair"
    | Snd(_, _) -> Abort "evalExpr snd: not a pair"
...
let check e =
  let rec check' (e : expr<'a>) (env : string list) : bool =
    ...
    | Fst(Pair(e1, e2, _),_)  ->  check' e1 env 
    | Fst(_,_)  ->  failwith "wrong fst argument"
    | Snd(Pair(e1, e2, _),_)  ->  check' e2 env 
