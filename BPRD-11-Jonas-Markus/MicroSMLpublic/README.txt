...
A. Building the micro-SML command line compiler microsmlc:
   ...
   fsharpc --standalone -r:/Users/nh/fsharp/FsLexYacc.Runtime.dll Absyn.fs FunPar.fs FunLex.fs TypeInference.fs HigherFun.fs Machine.fs Contcomp.fs ParseTypeAndRun.fs MicroSMLC.fs -o microsmlc.exe
...
C. Building the virtual machine
  ...
  On Unix / Mac:
    Go to directory MsmlVM/MsmlVM where you find the C file
    msmlmachine.c
    
    gcc -Wall msmlmachine.c -o msmlmachine

  On Windows:
    You have to use the Visual Studio project MsmlVM and either
    compile for 32 or 64 bit.

Running the compiled test program on Mac:

  ./MsmlVM/msmlmachine test.out
