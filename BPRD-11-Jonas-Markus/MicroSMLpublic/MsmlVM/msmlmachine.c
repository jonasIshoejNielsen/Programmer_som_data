...
void printInstruction(word p[], word pc) {
  ...
  case PAIR:   printf("PAIR" WORD_FMT WORD_FMT, p[pc + 1], p[pc + 2]); break;
...
word mkheader(unsigned int tag, word length, unsigned int color) { 
...
word* allocate(unsigned int tag, word length, word s[], word sp) {
  int attempt = 1;
  do {
    word* free = freelist;
    word** prev = &freelist;
