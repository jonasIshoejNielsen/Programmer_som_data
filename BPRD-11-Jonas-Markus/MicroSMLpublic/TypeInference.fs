...
let rec typeToString t : string =
  ...
  | TypL t       -> "list"
  | TypE         -> "exn"
  | TypP (t1, t2) -> "("+(typeToString t1) + " * " + (typeToString t2)+")"
...
let rec unify t1 t2 : unit =
  ...
  | TypP (_,_), t ->  failwith ("type error: pair followed by " + typeToString t)
...
let rec typExpr (lvl : int) (env : tenv) (e : expr<'a>) : typ * expr<typ> =
  ...
  | Pair(e1, e2, _) -> 
    let t1, o1 = typExpr lvl env e1
    let t2, o2 = typExpr lvl env e2
    let typePair = TypP(t1, t2)
    (typePair, Pair(o1, o2, Some typePair))
  | Fst(Pair(e1, _, _), _) -> 
      let t1, o1 = typExpr lvl env e1
      (t1, o1)
  | Fst(_, _) -> failwith "Wrong fst argument"
  | Snd(Pair(_, e2, _), _) -> 
      let t1, o1 = typExpr lvl env e2
      (t1, o1)
