...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | Prim1(ope, e1) -> 
      eval e1 (fun v1 -> fun econt2 -> 
          match (ope, v1) with
          | ("sqr", Int i1) -> 
              cont (Int(i1*i1)) econt2 
          | ("even", Int i1) -> 
              if i1%2 = 0 then cont (Int i1) econt2
              else econt2 ()
          | ("multiples", Int i1) -> 
              let rec mull i = cont (Int (i*i1)) (fun () -> mull (i+1))
              mull 1
          | _ -> Str "unknown prim1")
          econt
    | And(e1, e2) -> 
...
let ex11_8_1 = Seq(Every(Write(Prim("+",Prim("*", FromTo(1, 4), CstI 2), CstI 1))), Prim("+",Prim("*", FromTo(1, 4), CstI 2), CstI 1));
//run ex11_8_1;;


let ex11_8_2 = 
  let nr1 = Prim("+", Prim("*", FromTo(2, 4), CstI 10), FromTo(1, 2));
  Seq(Every(Write(nr1)), nr1);
//run ex11_8_2;;

//(ii)
let ex11_8_3 = Prim("<", CstI 50, Prim("*", FromTo(1, 10), CstI 7))
//run ex11_8_3;;

//(iii)

let ex11_8_4 = 
  let nr1 = Prim1("sqr", FromTo(3, 6));
  Seq(Every(Write(nr1)), nr1);
//run ex11_8_4;;

let ex11_8_5 = 
  let nr1 = Prim1("even", FromTo(1, 7));
  Seq(Every(Write(nr1)), nr1);
//run ex11_8_5;;

//(iv)
let ex11_8_6 = 
  let nr1 = Prim1("multiples", FromTo(3, 4));
  Seq(Every(Write(nr1)), nr1);;
//run ex11_8_6;;
