...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Letfun(f, argLst, fBody, letBody) ->      //changed
      let bodyEnv = (f, Closure(f, argLst, fBody, env)) :: env 
      eval letBody bodyEnv
    | Call(Var f, eArgList) ->                  //changed
      let fClosure = lookup env f
      match fClosure with
      | Closure (f, argNameLst, fBody, fDeclEnv) ->      
        let fBodyEnv =
            List.zip argNameLst eArgList
            |> List.fold (fun acc (name, eArg) -> (name, Int(eval eArg env)) :: acc) ((f, fClosure) :: fDeclEnv)
        eval fBody fBodyEnv
        ...
        ("pow",["x"; "n"],
         If
           (Prim ("=",Var "n",CstI 0),CstI 1,
            Prim
              ("*",Var "x",Call (Var "pow",[Var "x"; Prim ("-",Var "n",CstI 1)]))),
