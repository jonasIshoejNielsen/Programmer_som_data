...
let fromString (str : string) : expr =
    let lexbuf = (*Lexing. insert if using old PowerPack *)Lexing.LexBuffer<char>.FromString(str)
    try 
...
let fromFile (filename : string) =
    ...
    let lexbuf = (* Lexing. insert if using old PowerPack *) Lexing.LexBuffer<char>.FromTextReader reader
    try 
      FunPar.Main FunLex.Token lexbuf
    with 
      | exn -> let pos = lexbuf.EndPos 
