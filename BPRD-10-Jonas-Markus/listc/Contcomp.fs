...
let addZERO instrRev instr lab3 C : instr list =
    match C with
    | GOTO lab1::Label lab2::c2 when (lab2 = lab3) -> 
        (instrRev lab1)::(Label lab2)::c2
    | _ -> instr lab3 :: C 
let addIFZERO  = addZERO IFNZRO IFZERO
let addIFNZRO = addZERO IFZERO IFNZRO
...
let rec addCST i C =
    ...
    | (i1, CSTI i2::LT::C1)         -> (if (i1 < i2) then 1 else 0) |> addCST2 C1
    | (i1, CSTI i2::SWAP::LT::C1)   -> (if (i1 > i2) then 1 else 0) |> addCST2 C1
    | (i1, CSTI i2::EQ::C1)         -> (if (i1 = i2) then 1 else 0) |> addCST2 C1
    | _                     -> CSTI i :: C
