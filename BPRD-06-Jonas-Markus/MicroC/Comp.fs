...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) : instr list = 
    ...
    | Switch (e1, caseLst) ->
        let res = cExpr e1 varEnv funEnv
        let labend  = newLabel()
        let rec HandleCases caseLst=
            match caseLst with
            | [] -> [Label labend] 
            | Case (e2, stmt1)::rest -> 
                let labelse = newLabel()
                [DUP] @ cExpr e2 varEnv funEnv @ [EQ] @ [IFZERO labelse] 
                @ cStmt stmt1 varEnv funEnv @ [GOTO labend]
                @ [Label labelse] @ (HandleCases rest)
            | _ -> failwith "Switch contains statements that arent a case"
        res @ HandleCases caseLst @ [INCSP -1]
    | Case _ -> failwith "Used a case outside of a switch";
